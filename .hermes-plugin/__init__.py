"""Hermes Agent registration for the `harness` skills plugin.

Registers the five harness skills with Hermes' native skill loader so
`skill_view("harness:<name>")` can load them on demand.

Unlike superpowers, this plugin injects no session bootstrap context: the
harness skills are task-triggered (you reach for them when auditing or
documenting your harness), so there is nothing worth paying for on every first
turn.
"""
...
_SENTINEL = ("ai-context", "SKILL.md")


def _skills_dir() -> str:
    """Locate the stock skills/ tree for either supported install layout.

    - git-clone install (`hermes plugins install dEitY719/harness-skills`):
    ...
    for cand in candidates:
        if os.path.isfile(os.path.join(cand, *_SENTINEL)):
            return cand
    raise RuntimeError(
        "harness plugin: cannot find the skills/ tree "
