...
2. **Step 2 — 에이전트 2개 병렬 실행.** Agent 도구로 두 에이전트를 **한 메시지 안에서
   ...
   - **Agent 1 (README.md)** — 로드한 프롬프트를 분석해 한국어로 쓴다. 필수 항목은
     ...
     ④ 특징(병렬 실행, 쓰기 권한, 도메인 특화 등 주목할 설계 특성). 적절한 곳에는
     요약 표를 쓴다. 권장 골격은 `# /<skill-name> - <Title>` → 내장 스킬임을 밝히는
     한 줄 설명 → `## 동작 요약` → `## Phase N: ...` → `## 특징` 이지만,
     **고정 템플릿을 강요하지 말고 대상 스킬의 실제 단계에 맞춰 제목을 조정**한다.
   - **Agent 2 (PROMPT.md)** — 원문 프롬프트를 그대로 쓴다. 감싸는 제목도, 코드 펜스도
     붙이지 않는다. 원시 내용만 있는 그대로.
3. **Step 3 — 확인 및 판정.** 두 에이전트가 모두 끝나기를 기다린 뒤 결정적 판정을 낸다.
