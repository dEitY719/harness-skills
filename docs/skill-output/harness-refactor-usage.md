...
> **한 줄 요약** — 감사 리포트를 받아 low-risk 항목만 `args.changes` / `args.rejected` 로 분류한 뒤, 플러그인에 이미 배포된 워크플로우를 실행해 하네스를 정리합니다.

> NOTE: **미실행 예시** — 이 스킬은 워크플로를 실행해 사용자 하네스를 실제로 변경하므로 문서화 목적으로 실행하지 않았습니다. 명령·분류 규칙·출력 형식은 skills/harness-refactor/SKILL.md 인용이며 실행 로그가 아닙니다.

```
harness-legacy-check 리포트  ──▶  /harness:harness-refactor  ──▶  harness-refactor.js 실행 (args.changes/args.rejected)
```
...
- **워크플로우 실행** — `Workflow({ name: 'harness:harness-refactor', args: { changes, rejected } })`.
  ...
  `export const meta` 가 Pre-flight → Apply Changes → Verify → Final Report 4개 phase 를
  선언한다.
- **Pre-flight → Apply Changes → Verify** — Pre-flight 는 대상 파일 존재 확인과 archive
  디렉토리 생성만, Apply Changes 는 `parallel()` 로 비중첩 파일 그룹별 에이전트, Verify 는
  `wc -l` 비교와 `references/` 생성 확인.
